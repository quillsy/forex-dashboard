...
def parse_hicp(payload, *, now=None):
    ...
    expected = {'freq': 'M', 'unit': 'RCH_A', 'coicop18': 'TOTAL', 'geo': 'EA21'}
    ...
    return {'value': value, 'date': date.isoformat(), 'reference_period': period,
            ...
            'series_id': 'prc_hicp_minr:M.RCH_A.TOTAL.EA21', 'frequency': 'monthly',
...
def fetch_hicp(*, now=None, session=None):
    checked = now or datetime.now(timezone.utc)
    response = (session or requests).get(EUROSTAT_BASE + 'prc_hicp_minr',
        params={'freq': 'M', 'unit': 'RCH_A', 'coicop18': 'TOTAL', 'geo': 'EA21',
                'sinceTimePeriod': f'{checked.year - 1}-01', 'lang': 'EN'}, timeout=20)
    response.raise_for_status()
    return parse_hicp(response.json(), now=checked)
